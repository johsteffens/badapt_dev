...
/**********************************************************************************************************************/
// badapt_dev_jrn_layer_s

void badapt_dev_jrn_layer_s_setup( badapt_dev_jrn_layer_s* o, sz_t size_input, sz_t size_hidden, sz_t size_output )
{
...
void badapt_dev_jrn_arr_layer_s_setup( badapt_dev_jrn_arr_layer_s* o, sz_t size_arr, sz_t size_input, sz_t size_hidden, sz_t size_output )
{
    badapt_dev_jrn_arr_layer_s_set_size( o, size_arr );
    for( sz_t i = 0; i < o->arr_size; i++ )
    {
        if( !o->arr_data[ i ] ) o->arr_data[ i ] = badapt_dev_jrn_layer_s_create();
        badapt_dev_jrn_layer_s_setup( o->arr_data[ i ], size_input, size_hidden, size_output );
    }
}

// ---------------------------------------------------------------------------------------------------------------------

void badapt_dev_jrn_arr_layer_s_rotate( badapt_dev_jrn_arr_layer_s* o )
{
    if( o->arr_size <= 1 ) return;

    badapt_dev_jrn_layer_s* last_layer = o->arr_data[ o->arr_size - 1 ];
...
void badapt_dev_jrn_s_minfer( badapt_dev_jrn_s* o, const bmath_vf3_s* in, bmath_vf3_s* out )
{
    ...
    {
        badapt_dev_jrn_arr_layer_s_setup( &o->arr_layer, o->size_unfolded, o->size_input, o->size_hidden, o->size_output );
    }

    badapt_dev_jrn_arr_layer_s_rotate( &o->arr_layer );
    badapt_dev_jrn_layer_s* prev_layer = o->arr_layer.arr_data[ 1 % o->arr_layer.arr_size ];
    badapt_dev_jrn_layer_s* this_layer = o->arr_layer.arr_data[ 0 ];
...
{
    badapt_dev_jrn_layer_s* layer0 = o->arr_layer.arr_data[ 0 ];
    ...
    {
        badapt_dev_jrn_layer_s* layer = o->arr_layer.arr_data[ i ];
...
badapt_adaptive* badapt_dev_jrn_builder_s_build( const badapt_dev_jrn_builder_s* o )
{
    ...
    badapt_dev_jrn_s* ern = badapt_dev_jrn_s_create();
...
void badapt_dev_jrn_s_test_recurrent_kjv()
{
    BCORE_LIFE_INIT();
    BCORE_LIFE_CREATE( badapt_problem_recurrent_kjv_s, problem );
    BCORE_LIFE_CREATE( badapt_dev_jrn_builder_s,           builder );
    BCORE_LIFE_CREATE( badapt_trainer_batch_s,         trainer );
