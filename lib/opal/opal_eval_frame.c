 ...
 */
static void frame_s_estimate_jacobian_en( const opal_frame_s* const_o, const bhvm_holor_adl_s* en, f3_t epsilon, bhvm_holor_mdl_s* jac_mdl )
{
    BLM_INIT();
 ...
 */
static void frame_s_estimate_jacobian_ada( const opal_frame_s* const_o, const bhvm_holor_adl_s* adl_en, f3_t epsilon, bhvm_holor_mdl_s* jac_mdl )
{
    BLM_INIT();
    opal_frame_s* o = BLM_CLONE( opal_frame_s, const_o );
    ASSERT( o->setup );
...
opal_eval_frame_result_s* opal_eval_frame_plain_s_run( const opal_eval_frame_plain_s* o, opal_eval_frame_result_s* result )
{
    ...
    opal_frame_s* frame0 = BLM_CREATE( opal_frame_s );
    ...
    {
        opal_frame_s* frame1 = BLM_CREATE( opal_frame_s );
        bcore_bin_ml_a_copy( frame1, frame0 );
        frame0 = frame1;
    }

    /// test copying
    opal_frame_s* frame = BLM_CLONE( opal_frame_s, frame0 );
    ...
    if( o->param.verbosity >= 10 )
    {
        ...
        opal_frame_s_disassemble_to_sink( frame, o->param.log );
    ...
    if( o->param.jacobian_test )
    {
        ...
        if( opal_frame_s_get_size_en( frame ) )
        {
            ...
            BFOR_SIZE( i, opal_frame_s_get_size_en( frame ) )
            {
                BLM_INIT();
        ...
        if( opal_frame_s_get_size_ada( frame ) )
        {
            ...
            BFOR_SIZE( i, opal_frame_s_get_size_ada( frame ) )
            {
                BLM_INIT();
 ...
 *  jac_mdl stores jacobians in the form [in-channels][out-channels]
 */
static void frame_cyclic_s_estimate_jacobian_en( const opal_frame_cyclic_s* const_o, const bhvm_holor_adl_s* en, f3_t epsilon, bhvm_holor_mdl_s* jac_mdl )
{
    BLM_INIT();
...
opal_eval_frame_result_s* opal_eval_frame_cyclic_s_run( const opal_eval_frame_cyclic_s* o, opal_eval_frame_result_s* result )
{
    ...
    opal_frame_cyclic_s* frame_cyclic0 = BLM_CREATE( opal_frame_cyclic_s );
    opal_frame_cyclic_s_setup_from_frame( frame_cyclic0, frame, unroll_size );

    /// test frame recovery/copying
    if( o->param.recovery_test)
    {
        opal_frame_cyclic_s* frame_cyclic1 = BLM_CREATE( opal_frame_cyclic_s );
        bcore_bin_ml_a_copy( frame_cyclic1, frame_cyclic0 );
        frame_cyclic0 = frame_cyclic1;
    }

    opal_frame_cyclic_s* frame_cyclic  = BLM_CLONE( opal_frame_cyclic_s, frame_cyclic0 );
...
vd_t opal_eval_frame_signal_handler( const bcore_signal_s* o )
{
    switch( bcore_signal_s_handle_type( o, typeof( "opal_eval_frame" ) ) )
    {
        case TYPEOF_init1:
        {
        }
