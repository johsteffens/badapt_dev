...
void lion_frame_s_estimate_jacobian_en( const lion_frame_s* const_o, f3_t epsilon, bhvm_holor_mdl_s* jac_mdl )
{
    ...
    ASSERT( const_o->setup );
    ASSERT( o->setup );

    /// copy input and run ap
    BFOR_SIZE( i, o->size_en ) bhvm_value_s_cpy( &lion_frame_s_get_ap_en( ( lion_frame_s* )const_o, i )->v, &lion_frame_s_get_ap_en( o, i )->v );
    bhvm_mcode_frame_s_track_run( o->mcf, TYPEOF_track_ap );

    bhvm_holor_adl_s* ref = BLM_CREATE( bhvm_holor_adl_s );
    bhvm_holor_adl_s_set_size( ref, lion_frame_hidx_s_get_size( &o->hidx_ex ) );
    BFOR_EACH( k, ref ) ref->data[ k ] = bhvm_holor_s_clone( lion_frame_s_get_ap_ex( o, k ) );

    bhvm_holor_mdl_s_clear( jac_mdl );
    bhvm_holor_mdl_s_set_size( jac_mdl, lion_frame_hidx_s_get_size( &o->hidx_en ) );
    ...
    BFOR_SIZE( i, o->size_en )
    {
        ...
        bhvm_holor_adl_s_set_size( jac_adl, lion_frame_hidx_s_get_size( &o->hidx_ex ) );
...
void lion_frame_s_estimate_jacobian_ada( const lion_frame_s* const_o, f3_t epsilon, bhvm_holor_mdl_s* jac_mdl )
{
    ...
    bhvm_holor_adl_s* ref = BLM_CREATE( bhvm_holor_adl_s );
    bhvm_holor_adl_s_set_size( ref, o->size_ex );
    BFOR_EACH( k, ref ) ref->data[ k ] = bhvm_holor_s_clone( lion_frame_s_get_ap_ex( o, k ) );
